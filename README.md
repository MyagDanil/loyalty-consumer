![Typing SVG](https://readme-typing-svg.herokuapp.com?color=%2336BCF7&lines=Golang+dev+from+Russia)
...
чёткое разделение слоёв (handler, store, cache, kafka, config).
...
Пайплайн обработки:
1. Consumer читает сообщение из Kafka.
2. Handler десериализует JSON в ```models.Transaction``` и валидирует данные.
3. Открывается транзакция в PostgreSQL:
- создаётся запись о транзакции;
- обновляется баланс пользователя.
4. Выполняется ```COMMIT``` или ```ROLLBACK``` при ошибке.
5. В горутине читается актуальный баланс из БД и записывается в Redis как кеш.
